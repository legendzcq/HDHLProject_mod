...
- (void)layoutSubviews
{
    ...
    // 1.状态标签
    self.statusLabel.frame = CGRectMake(0, (self.height-statusHeight)/2.0, statusWidth, statusHeight);
...
//#warning 这句代码修复了，top值不断累加的bug
                    self.scrollView.mj_contentInsetTop = MAX(0,self.scrollView.mj_contentInsetTop - self.mj_height);
                }];
            } else {
                // 执行动画
